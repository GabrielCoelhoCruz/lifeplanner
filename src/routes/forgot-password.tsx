...
function ForgotPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Do not reveal whether the email is registered. Show the same
      // calm confirmation state regardless of the result.
      if (error) {
...
function ConfirmationState() {
  return (
    <div className="flex flex-col items-start">
      ...
      <p className="mt-2 text-sm text-text-secondary leading-relaxed">
        Se esta conta existir, você receberá um link em instantes.
      </p>
      <Link
        to="/login"
        className="mt-8 text-sm font-medium text-accent hover:text-accent-hover transition-colors"
      >
        Voltar para o login
      </Link>
    </div>
